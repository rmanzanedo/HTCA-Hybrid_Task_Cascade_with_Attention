...
class ModelInspector(object):
  ...
  def saved_model_inference_for_transformer(self, imgs, driver, layers, type='train', **kwargs):
      ...
      for n, img in enumerate(imgs):
          ...
          x = [x.astype(np.uint8)]
          detections_feat, detections_bs, back_feats = driver.serve_images(x, get_features)
          ...
          for k, i in zip(layers, detections_feat):
              ...
              output = add_to_dict(output, k, v)
          # back_feats.reverse()
          for k, i in zip(layers,back_feats):
              v = postprocessing(i[0])
              output1 = add_to_dict(output1, k, v)

      return output, output_detections, output1
