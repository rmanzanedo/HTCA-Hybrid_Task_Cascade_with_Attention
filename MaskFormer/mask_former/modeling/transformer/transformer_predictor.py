...
class TransformerPredictor(nn.Module):
    ...
    def forward(self, x, mask_features):
        ...
        # print('memory shape input decoder: {}'.format(memory.shape))
        # print('hs shape out decoder: {}'.format(hs.shape))
        # print('its memory equals to src: {}'.format(torch.all(memory.eq(self.input_proj(src)))))

        if self.mask_classification:
            outputs_class = self.class_embed(hs)
            out = {"pred_logits": outputs_class[-1]}
            # print('cls pred shape: {}'.format(outputs_class.shape))
        else:
            out = {}
        ...
        if self.aux_loss:
            ...
            # print('mask segmentation shape: {}'.format(outputs_seg_masks.shape))
        else:
