...
def bifpn_sum_config():
  ...
  # p.nodes = [
  #   {'feat_level': 6, 'inputs_offsets': [4, 5]},  # for P6'
  #   {'feat_level': 5, 'inputs_offsets': [3, 6]},  # for P5'
  #   {'feat_level': 4, 'inputs_offsets': [2, 7]},  # for P4'
  #   {'feat_level': 3, 'inputs_offsets': [1, 8]},  # for P3'
  #   {'feat_level': 2, 'inputs_offsets': [0, 9]},  # for P2"
  #   {'feat_level': 3, 'inputs_offsets': [1, 9, 10]},  # for P3"
  #   {'feat_level': 4, 'inputs_offsets': [2, 8, 11]},  # for P4"
  #   {'feat_level': 5, 'inputs_offsets': [3, 7, 12]},  # for P5"
  #   {'feat_level': 6, 'inputs_offsets': [4, 6, 13]},  # for P6"
  #   {'feat_level': 7, 'inputs_offsets': [5, 14]},  # for P7"
  # ]
  ###################################################################### Original #####################################
  p.weight_method = 'sum'
  return p
...
def efficientdet(features, model_name=None, config=None, **kwargs):
  ...
  ###################################################################### Mi codigo #####################################
  # feat_for_semantic_segmentation = fpn_feats.pop(2, None)
  # config.min_level = 3
  # kwargs['min_level'] = 3
  ...
  class_outputs, box_outputs = build_class_and_box_outputs(fpn_feats, config)
  logging.info('backbone+fpn+box params/flops = {:.6f}, {:.9f}'.format(
      *utils.num_params_flops()))
